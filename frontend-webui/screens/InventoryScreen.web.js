...
export default function InventoryScreen({ apiBaseUrl, session }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [cyRes, foRes] = await Promise.all([
        fetch(`${apiBaseUrl}/cylinders?company=${activeCompany}`, { headers: { Authorization: token } }),
        fetch(`${apiBaseUrl}/foils?company=${activeCompany}`, { headers: { Authorization: token } }),
      ]);
  ...
  const handleAddItem = async () => {
    ...
    try {
      const endpoint = activeTab === 'cylinder' ? `${apiBaseUrl}/cylinders` : `${apiBaseUrl}/foils`;
  ...
  return (
    <View style={styles.container}>
      ...
      <WebCard title={`${activeTab === 'cylinder' ? 'Cylinder Inventory' : 'Foil Stock Inventory'}`}>
        ...
        {activeTab === 'cylinder' && filteredCylinders.map((item) => (
        ...
        {activeTab === 'foil' && filteredFoils.map((item) => (
          <View key={item._id} style={styles.row}>
            <View style={{ flex: 1 }}>
              <Text style={styles.rowTitle}>{item.productName}</Text>
